  ...
  function save() {
    ...
    startTransition(async () => {
      try {
        ...
        setMessage("Pesos salvos. Novos scores e a próxima organização automática usarão esta prioridade.");
      } catch (err) {
  ...
  return (
    <section className="mt-6 rounded-2xl border border-indigo-400/20 bg-indigo-400/[0.04] p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div><p className="text-xs font-bold uppercase tracking-[0.18em] text-indigo-300">P3 · Personalização</p><h3 className="mt-2 text-lg font-black text-indigo-100">Pesos do score por projeto</h3><p className="mt-2 max-w-2xl text-xs leading-5 text-slate-400">Ajuste o que mais importa para este set. Peso zero desliga o fator; os fatores sem dados continuam fora do cálculo.</p></div>
      ...
      {total !== 100 ? <p className="mt-3 text-xs text-amber-200">A soma recomendada é 100%. Você pode salvar assim, mas a média usará os valores relativos informados.</p> : null}
      {message ? <p className="mt-3 rounded-lg border border-emerald-300/20 bg-emerald-300/10 px-3 py-2 text-xs text-emerald-100">{message}</p> : null}
      {error ? <p className="mt-3 rounded-lg border border-rose-300/20 bg-rose-300/10 px-3 py-2 text-xs text-rose-100">{error}</p> : null}
      <div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={() => setWeights(DEFAULT_WEIGHTS)} className="rounded-lg border border-white/10 px-3 py-2 text-xs text-slate-300">Restaurar padrão</button><button type="button" onClick={save} disabled={pending} className="rounded-lg bg-indigo-300 px-4 py-2 text-xs font-bold text-slate-950 disabled:opacity-60">{pending ? "Salvando..." : "Salvar pesos"}</button></div>
    </section>
  );
}
